...
# $output = az group create `
 --name 'rg_christianhoumann' `
 --location 'westeurope'
 Throw-WhenError -output $output


 $output = az storage account create --name "cethdag1storage" --resource-group "rg_christianhoumann"
 Throw-WhenError -output $output

 $output = az functionapp create --name "cethfunctionapp" --resource-group "rg_christianhoumann" --storage-account "cethdag1storage"
 Throw-WhenError -output $output

 $output = az appservice plan create --name "gefl" --resource-group "rg_christianhoumann"
 Throw-WhenError -output $output
